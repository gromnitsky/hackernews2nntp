...
(function() {
  ...
  Message = (function() {
    ...
    Message.TemplateGet = function(name) {
      return fs.readFileSync(path.resolve(__dirname, '..', 'template', "" + name + ".txt")).toString();
    };
    ...
    Message.prototype.render = function(_tdd_hash) {
      ...
      if (this.json_data.type === 'poll') {
        return this.polparts_collect().then(function(r) {
          json.mail.polparts = r;
          if (_tdd_hash) {
            _tdd_hash.polparts = r;
          }
          return Mustache.render(Message.TemplateGet(json.type), json);
        });
      } else {
        return Message.HTML_filter(this.json_data.text).then(function(r) {
